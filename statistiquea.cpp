...
#include <algorithm>
using namespace  std;
Statistiquea::Statistiquea()
{



}
QChartView * Statistiquea::Preparechart(QString type)
{
    QSqlQuery query;
    ...
    if(query.exec("select datea,sum(prix) from achat group by datea"))
    {
    while(query.next())
      {
    ...
    chart->setTitle("Revenu Par rapport a date");
        ...
        while(query.next())
          {
            ...
            *set0 << query.value(1).toFloat() ;
            series->append(set0);
                 ...
                 query.exec(" select max(prix) from achat");
